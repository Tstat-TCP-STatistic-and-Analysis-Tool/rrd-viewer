...
# Run relative to the script's own directory regardless of how the web
# server / FastCGI wrapper invokes it, so the relative $RRD_DATA,
# $RRD_GALLERY and $IMG_DIR paths below always resolve correctly.
...
# specify path to the root of the rrd database tree: every immediate
# (or nested) sub-directory of $RRD_DATA that contains *.rrd files is
# offered as a selectable "trace". To add a new trace, just mount/copy
# its folder under $RRD_DATA -- no code change required.
# Overridable via the RRD_DATA_DIR environment variable.
$RRD_DATA = $ENV{RRD_DATA_DIR} || 'rrd_data';

# where "add to gallery" snapshots are stored; served back by the web
# server at /rrd_gallery/
$RRD_GALLERY = $ENV{RRD_GALLERY_DIR} || 'rrd_gallery';

# where generated graphs (png/eps) are written; served back by the web
# server at /rrd_images/
$IMG_DIR = $ENV{RRD_IMG_DIR} || 'rrd_images';

# these are written to at runtime, make sure they exist
foreach my $d ( $RRD_GALLERY, $IMG_DIR ) {
    mkdir $d unless -d $d;
}
...
#   open FIND_DIR, "find $RRD_DATA/ -type d | sort |";
   ...
   while (<FIND_DIR>) { 
     ...
     if( @glob = glob("$_/*.rrd") ) {
        ...
        s/^.*$RRD_DATA\///g;    
        $short = $_;
        $found = 1 if ($dir_check eq $short || $dir_check eq $long);
...
sub validateVar {
   ...
   $found = 1 if exists $description{$var_check};
...
sub main {
    ...
    if (url_param()) {	     
	     ...
	     #NEWVAR
             ...
             #verb ("Valid pDir $pDir ".validateDir($pDir));
             $pDir = $RRD_DATA unless validateDir($pDir); # -MMM- pDir sanity check
	     $pDir = "$RRD_DATA/$pDir" unless $pDir =~ $RRD_DATA;

             $pVar  = url_param('var') || 'NULL'; 
             #verb ("pVar = $pVar");
             $pVar  = 'NULL' unless validateVar($pVar); # -MMM- pVar sanity check
...
#        49 => ["Unclassified",'blue'],       
   ...
   L7_WWW_num => {
        ...
        15 => ["Facebook (Zero)",           'dmagenta', 9],
        11 => ["Dropbox (TLS)",   'yellowV2', 10],
        12 => ["Microsoft (TLS)",             'msblue', 11],
        13 => ["Apple (TLS)",             'pink', 12],
         4 => ["Other",             'blue', 14],
         6 => ["Other (TLS)",               'lblue', 15],
        14 => ["Instagram (TLS)",             'dorangeV2', 13],
   },
   www_bitrate => {
         0 => ["GET",               'mviolet', 0],
         1 => ["POST",              'yellow', 1],
